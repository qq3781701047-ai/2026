...
# 输出目录配置
OUTPUT_BASE_SUBDIR = os.path.join('股票数据', '打板专题数据', '同花顺涨跌停榜单')
...
OUTPUT_FIELDS = [
    ...
    'lu_desc', 'limit_type', 'tag', 'status', 'first_lu_time', 'last_lu_time', 
    'first_ld_time', 'last_ld_time', 'limit_order', 'limit_amount', 
    'turnover_rate', 'free_float', 'lu_limit_order', 'limit_up_suc_rate', 
    'turnover', 'rise_rate', 'sum_float', 'market_type'
]
...
def run_script():
    write_log(f"--- {SCRIPT_NAME}.py 开始执行 (采集同花顺涨跌停榜单) ---")
    
    trade_dates = load_trade_dates()
    if not trade_dates: return

    # 同花顺数据历史从 20231101 开始提供
    START_DATE = '20231101'
    ...
    write_log(f"1. 准备按日期采集同花顺榜单。时间轴范围: {trade_dates_to_pull[0]} 到 {trade_dates_to_pull[-1]}。")

    # 按天调用 limit_list_ths 接口获取全市场快照并分区落地
    for date_str in tqdm(trade_dates_to_pull, desc=f"分区落地 [{SCRIPT_NAME}]"):
        try:
            # 核心逻辑：按日期调用 API 获取当日涨跌停榜单
            # limit_list_ths 没有 limit 参数，需要按日期循环
            df_data = PRO_API.limit_list_ths(trade_date=date_str)
            
            # 引入延时机制 (注意：同花顺接口频率更高，但仍需控制)
            time.sleep(0.1) 
