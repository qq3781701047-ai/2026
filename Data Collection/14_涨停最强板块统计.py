...
# 输出目录配置
OUTPUT_BASE_SUBDIR = os.path.join('股票数据', '打板专题数据', '涨停最强板块统计')
...
# 定义输出字段
OUTPUT_FIELDS = [
    'ts_code', 'name', 'trade_date', 'days', 'up_stat', 'cons_nums', 
    'up_nums', 'pct_chg', 'rank'
]
...
def run_script():
    write_log(f"--- {SCRIPT_NAME}.py 开始执行 (采集涨停最强板块统计) ---")
    
    trade_dates = load_trade_dates()
    if not trade_dates: return

    # 板块统计数据历史较短，但我们从合理的较早日期开始拉取，并按天循环
    START_DATE = '20150101'
    ...
    write_log(f"1. 准备按日期采集最强板块统计数据。时间轴范围: {trade_dates_to_pull[0]} 到 {trade_dates_to_pull[-1]}。")

    # 按天调用 limit_cpt_list 接口获取全市场快照并分区落地
    for date_str in tqdm(trade_dates_to_pull, desc=f"分区落地 [{SCRIPT_NAME}]"):
        try:
            # 核心逻辑：按日期调用 API 获取当日涨停最强板块数据
            df_data = PRO_API.limit_cpt_list(trade_date=date_str)
