...
# -------------------------- T0: 元信息 (Meta Info) --------------------------
SCRIPT_NAME = "43_融资融券交易明细"
INTERFACE_NAME = "margin_detail" # 接口：margin_detail
PARTITION_KEY = 'trade_date' 
# 新目录：D:\2025\raw\股票数据\两融及转融通\融资融券交易明细\
OUTPUT_BASE_SUBDIR = os.path.join('股票数据', '两融及转融通', '融资融券交易明细') 
OUTPUT_FORMAT = 'csv' 
...
MAX_RETRY = 3 
# margin_detail 接口限制 5000次/分钟。设置为 0.35 秒的安全间隔。
API_SLEEP_SECONDS = 0.35 
PAGE_SIZE = 5000 # 设置合理的分页大小
DEFAULT_START_DATE = "19900101" 

# 所需字段
FIELDS = "trade_date,ts_code,name,rzye,rqye,rzmre,rqyl,rzche,rqchl,rqmcl,rzrqye,update_time,create_time"
...
def fetch_and_save_by_date(date_str: str) -> int:
    """
    按交易日调用 Tushare margin_detail 接口并分页采集全量数据。
    """
    ...
    while True:
        ...
        # 如果 API 失败 (current_page_rows==0 且重试失败)，或返回数据少于分页大小 (最后一页)
