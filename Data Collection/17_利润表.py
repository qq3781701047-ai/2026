...
# SCRIPT 17: 利润表 (income)
# -------------------------- T0: 元信息 --------------------------
SCRIPT_NAME = "17_利润表"
INTERFACE_NAME = "income"
PARTITION_KEY = 'end_date' # T4: 报告期分区
OUTPUT_BASE_SUBDIR = os.path.join('股票数据', '财务数据', '利润表')
...
def fatal_exit(script_name, message):
    ...
    # Tushare API返回重复数据是偶发情况，我们不再硬停止，而是记录错误并退出当前股票循环
    # sys.exit(1) # 硬停止改为软退出，以继续下一只股票
    raise Exception(message) # 使用异常传递错误信息，在主循环中捕获并跳过当前股票
...
def save_data_to_partition_csv(script_name, df, partition_value):
    ...
    try:
        if file_exists:
            ...
            # D:\2025\Data Collection\17_利润表.py:109 处的修正
...
def fetch_and_save_by_stock(ts_code, start_date, end_date):
    ...
    # 目的：Tushare API 偶尔会返回 end_date/report_type 相同但 ann_date 不同的重复行。
    # 我们应该保留 ann_date 最新的那一行。
    
    # 1. 确保 ann_date 是可排序的字符串
    if 'ann_date' in df_stock.columns:
        df_stock['ann_date'] = df_stock['ann_date'].fillna('00000000') # 填充空值以确保排序
    
    # 2. 按主键（ts_code, end_date, report_type）和 ann_date 排序，保留 ann_date 最晚的
    ...
    # 在强制去重后，理论上不应再失败。若失败，则表明存在更复杂问题。
