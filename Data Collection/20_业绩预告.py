...
MAX_RETRY = 3 
# Tushare forecast 接口限制 5000 次/分钟，0.35 秒的安全间隔是足够的
API_SLEEP_SECONDS = 0.35
LOG_DIR = os.path.join(BASE_DIR, 'logs', TODAY_DATE_STR)
os.makedirs(LOG_DIR, exist_ok=True)

# SCRIPT 20: 业绩预告 (forecast)
# -------------------------- T0: 元信息 --------------------------
SCRIPT_NAME = "20_业绩预告"
INTERFACE_NAME = "forecast"
PARTITION_KEY = 'ann_date' # T4: 按公告日期分区
OUTPUT_BASE_SUBDIR = os.path.join('股票数据', '财务数据', '业绩预告')
...
OUTPUT_FIELDS = [
    ...
    'net_profit_min', 'net_profit_max', 'last_parent_net', 'notice_times', 
...
def fatal_exit(script_name, message):
    ...
    # 业绩预告是按日查询，单个日期失败，我们使用异常传递错误信息，在主循环中捕获并跳过当前日期
...
def fetch_and_save_by_ann_date(start_date, end_date):
    ...
    for date_str in pbar:
        try:
            ...
            # 【关键修正点】移除了 df_day = df_day.rename(columns={'ann_date': PARTITION_KEY})，因为 ann_date == PARTITION_KEY
